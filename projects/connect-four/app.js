...
class Game {
    board = new Board();
    player = new Player();
    playAgain = document.getElementById("play-again").addEventListener("click", this.restart);
    ...
    changeChip() {
        ...
        if(chips[id + 7].classList.contains("taken") && !chips[id].classList.contains("taken")) {
    ...
    checkWin(player) {
        ...
        for(let i = 7; i < 35; i++) {
            if( //any chip belongs to the specified player
                chips[i].classList.contains(player) 
                && ( //AND the chips above and below and one further BELOW belong to the specified player
                ...
                this.grantWin();
                return;
            }else if( //any chip belongs to the specified player
                chips[i].classList.contains(player) 
                && ( //OR the chips above and below and one further ABOVE belong to the specified player
                    chips[i + 7].classList.contains(player) 
                    && chips[i - 7].classList.contains(player)
                    && chips[i - 14].classList.contains(player)
                )
        ...
        for(let row = 0; row < 2; row++) {
            for(let i = 16; i < 19; i++) {
                if( //any chip belongs to the specified player
                    chips[i].classList.contains(player) 
                    && ( //AND the chips diagonally down and to the right belong to the specified player
                    ...
                    chips[i].classList.contains(player) 
                    && ( //OR the chips diagonally up and to the left belong to the specified player
                        chips[i + 8].classList.contains(player) 
                        && chips[i - 8].classList.contains(player)
                        && chips[i - 16].classList.contains(player)
                    )
            ...
            for(let row = 0; row < 2; row++) {
                for(let i = 16; i < 19; i++) {
                    if( //any chip belongs to the specified player
                        chips[i].classList.contains(player) 
                        && ( //AND the chips diagonally down and to the left belong to the specified player
                        ...
                        chips[i].classList.contains(player) 
                        && ( //OR the chips diagonally up and to the right belong to the specified player
                            chips[i + 6].classList.contains(player) 
                            && chips[i - 6].classList.contains(player)
                            && chips[i - 12].classList.contains(player)
                        )
